...
runtime. It exposes exactly 12 fixed tools for health/catalog/recent reads,
task creation, team-template retrieval and validation, background run start,
...
- The desktop launcher opens the browser only after worker health and the HTML workspace marker are both ready. Startup rejects an unrelated Harness listener before local HTTP or support-service effects, binds later readiness checks to the initially verified PID and creation time, and requires an interpreter file before starting or replacing Harness. A LiteLLM or browser-helper failure leaves the local Harness control page available with a bounded, redacted diagnostic.
- The desktop launcher writes its four managed `.env.local` fields through same-directory atomic replacement, restores a reconstructable prior Harness after failed replacement, reuses unreconstructable provider state fail-closed, and never declares an existing service idle when its bounded 20-Run inspection is incomplete.
